...
// #include <Sources/evse_mcu_periph.h>
...
uint8_t Enc28J60Network::readByte(uint16_t addr)
{
  ...
  ether_cs_active();
    ...
    waitspi();*/
    SPI_transrecv(ETHERNET_SPI_BASE,ENC28J60_READ_BUF_MEM);
    // read data
    data = SPI_transrecv(ETHERNET_SPI_BASE,0x00);
    //CSPASSIVE;
    ether_cs_inactive();
...
void Enc28J60Network::writeByte(uint16_t addr, uint8_t data)
{
  ...
  #else
    // issue write command
      SPI_transrecv(ETHERNET_SPI_BASE,ENC28J60_WRITE_BUF_MEM);
    // write data
      SPI_transrecv(ETHERNET_SPI_BASE,data);
  #endif
    //CSPASSIVE;
    ether_cs_inactive();
}
...
Enc28J60Network::readOp(uint8_t op, uint8_t address)
{
  ...
  #else
    // issue read command
    SPI_transrecv(ETHERNET_SPI_BASE,op | (address & ADDR_MASK));
    // read data
    data = SPI_transrecv(ETHERNET_SPI_BASE,0x00);
    // do dummy read if needed (for mac and mii, see datasheet page 29)
    if(address & 0x80)
...
void ether_cs_active(void)
{
    GPIOPinWrite(ETHERNET_SPI_PORT_BASE, ETHERNET_CS_PIN, GPIO_LOW);
}

void ether_cs_inactive(void)
{
